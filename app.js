...
function renderAssets(promptId){const list=assets.filter(a=>a.promptId===promptId).sort((a,b)=>b.created-a.created);$('#assetGallery').innerHTML=list.length?list.map(a=>`<article class="case-card ${a.mediaType==='video'?'video-asset-card':''}">${mediaHtml(a,a.name)}<button class="case-delete" data-delete-asset="${a.id}" title="删除">×</button><div class="case-meta"><strong>${esc(a.name)}</strong><span>${esc(a.type)}</span></div>${a.mediaType==='video'?'':`<div class="asset-actions"><button data-copy-asset="${a.id}">复制图片</button><button data-download-asset="${a.id}">下载</button></div>`}</article>`).join(''):`<div class="case-empty">还没有参考资产。复制图片或选择视频后保存即可。</div>`;}
