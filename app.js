...
function resetAssetForm(){const id=$('#assetPromptId').value;$('#assetForm').reset();$('#assetPromptId').value=id;pendingAssetImage='';pendingAssetFile=null;$('#assetImagePreview').hidden=true;$('#assetImagePreview').removeAttribute('src');$('#assetVideoPreview').hidden=true;$('#assetVideoPreview').removeAttribute('src');$('#assetPasteHint').hidden=false;}
...
$('#assetForm').addEventListener('submit',async e=>{e.preventDefault();if(!pendingAssetImage){alert('请先粘贴图片或选择图片 / 视频。');return}const submit=e.submitter;const originalText=submit.textContent;submit.disabled=true;const id=crypto.randomUUID();let media={image:pendingAssetImage,mediaType:pendingAssetFile?.type.startsWith('video/')?'video':'image'};try{if(pendingAssetFile&&window.cloudSync?.isSignedIn()){const size=(pendingAssetFile.size/1024/1024).toFixed(1);submit.textContent=`上传中 ${size}MB…`;showToast('视频正在上传，请不要关闭页面');media=await window.cloudSync.uploadFile(pendingAssetFile,'assets',id)}assets.unshift({id,promptId:'',name:$('#assetName').value.trim(),type:$('#assetType').value,note:$('#assetNote').value.trim(),...media,created:Date.now()});saveAssets();resetAssetForm();$('#assetDialog').close();renderGlobalAssets();showToast('视频案例上传并保存成功')}catch(error){console.error(error);alert(`上传失败：${error.message}\n\n请确认网络正常、SQL 已完整执行，并优先使用 100MB 内的 MP4 视频。`)}finally{submit.disabled=false;submit.textContent=originalText}});
